...
To query only some of the *Dog* items, you can apply a condition to the sort key \(for example, only the *Dog* items where *Name* begin with a letter that is within the range `A` through `K`\)\.
